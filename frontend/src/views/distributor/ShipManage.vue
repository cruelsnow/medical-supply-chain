    ...
    <el-dialog v-model="shipDialogVisible" title="确认发货" width="500px">
      <el-form :model="shipForm" label-width="100px">
        ...
        <el-form-item label="收货方名称" required>
          <el-input v-model="shipForm.newOwner" placeholder="输入医院名称" />
        </el-form-item>
...
const loadAssets = async () => {
  ...
  try {
    // 查询在库资产，过滤出经销商持有的
    const res = await assetApi.byStatus('IN_STOCK') as any
    if (res.success) {
      const data = res.data || []
      assetList.value = data
        .filter((item: any) => item.name && item.name.trim() !== '' && item.owner && item.owner.includes('经销商'))
        .sort((a: any, b: any) => {
...
const searchAsset = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // 经销商的在库资产可以发货
      if (res.data.status === 'IN_STOCK' && res.data.owner?.includes('经销商')) {
        assetList.value = [res.data]
      } else {
        ElMessage.warning('该资产不在可发货状态')
        assetList.value = []
      }
    }
  } catch (error) {
...
const showShipDialog = (asset: any) => {
  ...
  shipForm.newOwner = ''
  shipForm.newOwnerMSP = 'HospitalMSP'
